...
- If the target tool is not clear from context, ask which tool the prompt is for
...
## Workflow, Tool Routing, Diagnostics

### Intent Extraction

A usable prompt needs at least task, target tool, and output format. If one is missing and cannot be inferred, ask. The table lists what else to capture when relevant.
...
Non-Claude model names and parameters below date from skill v1.8.0 and are not re-verified; treat them as starting points.

---

**Claude (claude.ai, Claude API, Claude 5 / current Claude models)**

Do not assume one universal Claude default. Model lineup as of 2026-06 (apply the Model Recency Gate for anything newer): **Claude Opus 5** (`claude-opus-5`) is the default for complex agentic coding and enterprise work. **Claude Fable 5.1** (`claude-fable-5-1`) is for the hardest long-running autonomous work. **Claude Opus 5.5** (`claude-opus-5-5`) only when the user names it. **Claude Sonnet 5** (`claude-sonnet-5`) for speed plus frontier intelligence, **Claude Haiku 4.5** (`claude-haiku-4-5`) for fast, economical workloads. Ask which model only when the distinction changes the prompt.
...
- Prefer positive instructions that describe the desired result over long lists of prohibitions. Write rules at normal volume with the reason attached; capitalized MUST/NEVER emphasis makes current Claude models over-apply the rule.
...
- On Opus 5.5, test whether these scope and verification lines are still needed before keeping them.
...
- Short, clean instructions. These models reason internally, so do not add "think step by step" or other reasoning scaffolding.
- Prefer zero-shot first; add few-shot only if needed and tightly aligned.
- State what you want and what done looks like.
...
- Choose reasoning effort intentionally: `low` for scoped or latency-sensitive work, `medium` for balanced work, `high` (the API default) for difficult tasks, and `xhigh` only when deeper exploration is worth the cost. Grok 4.6 reasoning cannot be disabled.
...
- Ask which model is running before writing — Llama3, Mistral, Qwen2.5, CodeLlama all behave differently
...
- Include stop conditions so the agent does not loop indefinitely
...
- Name files and directories when the task is limited to them
...
- Name the file whenever the change is local to it
...
**Unknown tool:**
Identify the closest matching tool category from context. If genuinely unclear, ask: "Which tool is this for?" — then route accordingly.
...
- Two independent tasks in one prompt → split, deliver as Prompt 1 and Prompt 2 (for agents using Template M, one brief with a target state and acceptance criteria is enough)
...
- Scope is "the whole thing" for a non-agentic tool → decompose into sequential prompts
...
- Implicit length ("write a summary") → name the reader and purpose; use a number only when the format demands it (subject line, tweet)
- Missing domain context → add a one-line role only if it supplies context the prompt lacks
...
- No final report → add "At the end, list changed files and test results"
...
When the user's request references prior work, decisions, or session history — put this block at the start of the generated prompt.
...
**Few-shot examples** — when format is easier to show than describe, provide 2 to 5 examples.
...
**Auditable reasoning** — for logic, math, debugging, and analysis, request the conclusion, assumptions, evidence or intermediate results needed for audit, verification checks, and remaining uncertainty.
...
Check that the prompt uses the target tool's syntax and that the most important constraints come first. For Claude targets, state requirements plainly with their reason; reserve emphasis for a rule the target model demonstrably overlooks.
