...
| [H — ReAct + Stop Conditions](#template-h--react--stop-conditions) | Devin, Cline, and other non-Claude autonomous agents (Claude uses Template M) |
...
Response: Short enough to read on a phone without scrolling. Subject line included. No bullet points.
```
...
Narrowing: No technical implementation details. No wireframes. Readable in one sitting.
```
...
*Use for Devin, AutoGPT, and other non-Claude agents that take autonomous actions. For Claude Code use Template M. Stop conditions keep the agent from looping or expanding scope.*
...
Final report:
At the end, list every file changed and the test results.
```
...
**For Adapt tasks, ask if not already stated:**
