...
| 2 | **Two independent tasks in one prompt** | "explain AND rewrite this function" | Split into two prompts: explain first, rewrite second (agents using Template M can take both in one brief) |
...
| 6 | **Build-the-whole-thing** | "build my entire app" | Non-agentic tools: Prompt 1 (scaffold), Prompt 2 (core feature), Prompt 3 (polish). Agents: one Template M brief with target state and acceptance criteria |
...
| # | Pattern | Bad Example | Fixed |
|---|---------|------------|-------|
| 14 | **Missing output format** | "explain this concept" | "A few bullet points with a one-sentence summary at top, for a reader new to the topic" |
| 15 | **Implicit length** | "write a summary" | "Summary a busy manager can read in 30 seconds" |
| 16 | **Missing domain context** | (blank) | One line of role or domain context when it adds something: "Senior backend engineer, Node.js and PostgreSQL" |
...
| 19 | **Keyword-only prompt for Midjourney V8** | "logo, vector, flat, minimal" | "Minimal flat vector logo for a print studio, bold ink stroke forming a T-shirt outline, white background --ar 1:1 --raw" |
...
| 22 | **No stop condition for agents** | "build the whole feature" | Explicit stop conditions + a final report of changed files and test results |
...
| 33 | **No final report** | Agent ends without a summary | "At the end, list changed files and test results" |
